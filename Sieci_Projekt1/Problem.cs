...
namespace Sieci_Projekt1
{
    class Problem
    {
      ...
      //  public double ansMIn { get; set; }
        ...
        public NormalizedField[] ArgNormalize { get; set; }
        public NormalizedField YNormalize { get; set; }
        ...
        public void LoadCSVFileTestRegg()
        {
            ...
            csvRead.Close();
            for (int i = 0; i < values.Count; i++)
                values[i][0] = ArgNormalize[0].Normalize(values[i][0]);
            //Normalization(values);
            testSet = values.ToArray();
        }
        ...
        //{
        //    Max = new double[values[0].Length];
        //    Min = new double[values[0].Length];
        //    for (int i = 0; i < Max.Length; i++)
        //    {
        //        Max[i] = values.Max(v => v[i]);
        //        Min[i] = values.Min(v => v[i]);
        //    }
        //    //if (parameters.FunctionType == FunctionTypeEnum.Bipolar)
        //    //{
